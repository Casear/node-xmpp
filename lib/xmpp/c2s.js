...
function C2SServer(options) {
    ...
    if(this.options.availableSaslMechanisms&& Array.isArray(this.options.availableSaslMechanisms)){
        this.availableSaslMechanisms=this.options.availableSaslMechanisms
    }
...
C2SStream.prototype.authenticatemd5 = function(dict) {
    var self = this;
    var jid = new JID(dict.username, this.domains ? this.domains.toString() : "");
    
    this.emit('authenticate', { jid: jid, dict: dict, client: this },
                     function(error,dict) {

        if (!error&&dict) {
            self.jid = jid;
            self.authenticated = true;
            self.send(new ltx.Element("challenge", { xmlns: NS_XMPP_SASL }).
                      t(encode64('rspauth='+self.mechanism.rspAuth(dict))));
        } else if(error){
                self.emit('auth-failure', self.jid);
                self.send(new ltx.Element("failure", { xmlns: NS_XMPP_SASL }).
                          c('text').t(error.message));
        }
    });
};
C2SStream.prototype.AuthMD5hash = function(dict) {

    return this.mechanism.AuthMD5Value(dict);
}
C2SStream.prototype.onAuth = function(stanza) {
    if(!this.mechanism && stanza.attrs.mechanism){

        this.matchingMechs = this.mechanisms.filter(function(mech) {
            return mech.name === stanza.attrs.mechanism;
        });
        if (this.matchingMechs[0]) {
            this.mechanism = this.matchingMechs[0];
        }
    }
    if(this.mechanism){

        if(this.mechanism.name=='DIGEST-MD5'){
            
            if(stanza.is('response')){
                var content = stanza.getText();
                if(content){
                    var content = decode64(stanza.getText())
                    if(this.mechanism.response(content)){
                        this.mechanism.authServer(content , this);
                    }else{
                        this.emit('auth-failure', this.jid);
                        this.send(new ltx.Element("failure", { xmlns: NS_XMPP_SASL }).
                                  c('text').t('content response failed' + content));

                    }
                }else if(this.authenticated){
                    this.send(new ltx.Element("success", { xmlns: NS_XMPP_SASL }));
                     // incoming stream restart
            ...
            else if(stanza.is('auth')){
                this.send(new ltx.Element("challenge", { xmlns: NS_XMPP_SASL }).t(encode64(this.mechanism.serverChallenge(''))).root());
