...
function selectMechanism(offeredMechs, preferredMech, availableMech) {
	...
	byName[string.toUpperCase(mechClass.prototype.name)] = mechClass;
    });
    /* Any preferred? */
    preferredMech=string.toUpperCase(preferredMech);
...
function availableMechanisms(availableMech) {
    ...
    var mechanisms = [];
    if (availableMech&&Array.isArray(availableMech)) {
        availableMech.forEach(function(mech) {
            mech=string.toUpperCase(mech);
            if(byName[mech]){
                mechanisms.push(byName[mech])
            }
        });
    }
    return mechanisms;
