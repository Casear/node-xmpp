...
function availableMechanisms(availableMech) {
    ...
    var byName = {};
    mechClasses.forEach(function(mechClass) {
        byName[mechClass.prototype.name.toUpperCase()] = mechClass;
    });
    var mechanisms = [];
    if (availableMech&&Array.isArray(availableMech)&&availableMech.length!=0) {
        availableMech.forEach(function(mech) {
            mech=mech.toUpperCase();
            if(byName[mech]){
                mechanisms.push(new byName[mech])
            }
        });

    }else{
        mechanisms.push(new Plain())
...
DigestMD5.prototype.AuthMD5Value = function(dict) {
    ...
    if (dict.nonce && dict.qop) {
        ...
        var a1 = md5(dict.username  + ':' +
                     (dict.realm||'') + ':' +
                     dict.password) + ':' +
                     dict.nonce + ':' +
                     dict.cnonce +
                     (dict.authzid ? ':'+dict.authzid:"");
        
        var a2 = "AUTHENTICATE:" + dict['digest-uri'];
        if (dict.qop == 'auth-int' || dict.qop == 'auth-conf')
            a2 += ":00000000000000000000000000000000";
...
DigestMD5.prototype.rspAuth = function(dict) {
    ...
    var value = md5_hex(md5_hex(a1) + ':' +
                    dict.nonce + ':' +
                    dict.nc + ':' +
                    dict.cnonce + ':' +
                    dict.qop + ':' +
                    md5_hex(':'+dict['digest-uri']));
    
    return value;
};
DigestMD5.prototype.authServer = function(auth ,client) {
    var c = parseDict(auth);
    if(c){
        client.authenticatemd5(c);
    }
}
...
DigestMD5.prototype.serverChallenge = function(realm) {
    var dict = {};
    dict.realm = realm || "";
